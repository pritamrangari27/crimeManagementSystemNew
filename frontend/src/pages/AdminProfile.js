...
const AdminProfile = () => {
  ...
  const getInitials = () =>
    (user?.username || 'A').split(' ').map(n => n[0]).join('').toUpperCase().substring(0, 2);
        ...
        <div className="d-flex justify-content-between align-items-center mb-3 gap-2 flex-wrap"
          ...
          <div>
            <h2 className="fw-bold mb-0" style={{ color: '#1a1a1a', fontSize: '1.3rem' }}>
              <i className="fas fa-user-shield me-2" style={{ color: '#0ea5e9' }}></i>Admin Profile
            </h2>
              ...
              {isEditing ? (
                <Form onSubmit={handleSubmit}>
                  <Row className="g-3">
                    ...
                    <Col md={6}>
                      <Form.Group>
                        <Form.Label className="fw-semibold small mb-1"><i className="fas fa-building me-1" style={{ color: '#0ea5e9' }}></i> Department</Form.Label>
                        <Form.Control size="sm" type="text" name="department" value={formData.department} onChange={handleChange} placeholder="e.g., Crime Management Division" style={{ borderRadius: 8, border: '1.5px solid #e2e8f0' }} />
                      </Form.Group>
                    </Col>
                  </Row>
                  <div className="d-flex gap-2 mt-3 pt-2" style={{ borderTop: '1px solid #f1f5f9' }}>
                    ...
                    <Button size="sm" variant="outline-secondary" className="fw-bold px-3" style={{ borderRadius: 8 }}
                      onClick={() => { setIsEditing(false); setFormData({ username: user?.username || '', email: user?.email || '', phone: user?.phone || '', department: user?.department || '' }); }}>
                ...
                <>
                  ...
                  <div style={{ marginTop: 16, padding: '12px 14px', borderRadius: '10px', background: '#f0fdf4', border: '1px solid #d1fae5' }}>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                      {['Manage Criminals', 'Manage Police', 'Manage Stations', 'FIR Management', 'View Analytics'].map((p, i) => (
                        <span key={i} style={{ background: '#dcfce7', color: '#166534', borderRadius: 20, padding: '3px 10px', fontSize: '0.72rem', fontWeight: 600 }}>
                          <i className="fas fa-check me-1"></i>{p}
                        </span>
                      ))}
                    </div>
                  </div>
